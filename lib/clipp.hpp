...
#ifndef __io_clipp_HPP__
#define __io_clipp_HPP__
...
constexpr auto check_is_callable_without_arg(int) -> decltype(
  std::declval<Fn>()(),
  std::integral_constant<
    bool, std::is_same<Ret, typename std::result_of<Fn()>::type>::value>{});
...
template <class Fn, class Ret> struct is_callable;
...
template <class> constexpr auto check_is_input_range(long) -> std::false_type;
...
template <class> constexpr auto check_has_size_getter(long) -> std::false_type;
...
template <class T, class V> struct limits_clamped<T, V, false> {
  static T from(const V &v) { return T(v); }
};

template <class T, class V> inline T clamped_on_limits(const V &v) {
...
template <> struct make<unsigned char> {
...
template <> struct make<unsigned short int> {
...
template <> struct make<unsigned int> {
...
template <> struct make<unsigned long int> {
...
template <> struct make<unsigned long long int> {
  static inline unsigned long long int from(const char *s) {
    if (!fwd_to_unsigned_int(s)) return (0);
    return clamped_on_limits<unsigned long long int>(
      std::strtoull(s, nullptr, 10));
  }
};

template <> struct make<char> {
...
template <> struct make<short int> {
...
template <> struct make<std::string> {
  static inline std::string from(const char *s) { return std::string(s); }
};

template <class T, class V = T> class assign_value {
public:
  template <class X>
...
template <class T> class increment_by {
public:
...
template <class T> class map_arg_to {
public:
...
template <class T> class map_arg_to<std::vector<T>> {
public:
...
template <> class map_arg_to<bool> {
public:
...
template <class T> T make(const arg_string &s) {
...
subrange first_number_match(std::basic_string<C, T, A> s,
                            C digitSeparator = C(','), C decimalPoint = C('.'),
                            C exponential = C('e')) {
  using string_t = std::basic_string<C, T, A>;
...
template <class C, class T, class A>
subrange first_integer_match(std::basic_string<C, T, A> s,
                             C digitSeparator = C(',')) {
  using string_t = std::basic_string<C, T, A>;
...
template <class T> inline detail::map_arg_to<T> set(T &target) {
...
template <class T> inline detail::increment<T> increment(T &target) {
  return detail::increment<T>{target};
}

template <class T> inline detail::increment_by<T> increment(T &target, T by) {
  return detail::increment_by<T>{target, std::move(by)};
}

template <class T> inline detail::decrement<T> decrement(T &target) {
  return detail::decrement<T>{target};
}

namespace detail {

template <class Derived> class action_provider {
private:
...
public:
  ...
  template <class Target> Derived &set(Target &t) {
  ...
  template <class Target, class Value> Derived &set(Target &t, Value &&v) {
  ...
  template <class T, class... Ts> Derived &target(T &&t, Ts &&...ts) {
...
template <class Derived> class token {
public:
...
template <class T> inline T &operator%(doc_string docstr, token<T> &p) {
...
template <class T> inline T &operator%(token<T> &p, doc_string docstr) {
  return p.doc(std::move(docstr));
}
...
template <class T> inline T &doc(doc_string docstr, token<T> &p) {
  return p.doc(std::move(docstr));
}
...
class numbers {
public:
  explicit numbers(char decimalPoint = '.', char digitSeparator = ' ',
                   char exponentSeparator = 'e')
    : decpoint_{decimalPoint},
      separator_{digitSeparator}, exp_{exponentSeparator} {}
...
public:
  parameter()
    : flags_{}, matcher_{predicate_adapter{match::none}}, label_{},
      required_{false}, greedy_{false} {}
  ...
  explicit parameter(match_predicate filter)
    : flags_{}, matcher_{predicate_adapter{std::move(filter)}}, label_{},
      required_{false}, greedy_{false} {}

  explicit parameter(match_function filter)
    : flags_{}, matcher_{std::move(filter)}, label_{}, required_{false},
      greedy_{false} {}
  ...
  inline friend parameter &with_suffixes_short_long(const arg_string &shortsfx,
                                                    const arg_string &longsfx,
                                                    parameter &p) {
...
template <class Filter, class... Targets,
          class = typename std::enable_if<
            traits::is_callable<Filter, bool(const char *)>::value ||
            traits::is_callable<Filter, subrange(const char *)>::value>::type>
inline parameter opt_value(Filter &&filter, doc_string label,
                           Targets &&...tgts) {
  return parameter{std::forward<Filter>(filter)}
    .label(label)
    .target(std::forward<Targets>(tgts)...)
...
template <class Filter, class... Targets,
          class = typename std::enable_if<
            traits::is_callable<Filter, bool(const char *)>::value ||
            traits::is_callable<Filter, subrange(const char *)>::value>::type>
inline parameter opt_values(Filter &&filter, doc_string label,
                            Targets &&...tgts) {
  return parameter{std::forward<Filter>(filter)}
    .label(label)
    .target(std::forward<Targets>(tgts)...)
...
template <class... Targets> inline parameter any_other(Targets &&...tgts) {
...
class group : public detail::token<group> {
  ...
  template <class Param, class Group> struct child_t {
...
public:
  ...
  public:
    ...
    bool is_alternative(int minlevel = 0) const noexcept {
      ...
      return std::any_of(
        stack_.begin() + minlevel, stack_.end(),
  ...
  group &merge(group &&g) {
    children_.insert(children_.end(), std::make_move_iterator(g.begin()),
  ...
  size_type param_count() const {
    size_type c = 0;
    for (const auto &n : children_) { c += n.param_count(); }
    return c;
  }
...
template <class Action> void for_all_params(group &g, Action &&action) {
  for (auto &p : g) {
    if (p.is_group()) {
...
template <class Action> void for_all_params(const group &g, Action &&action) {
  for (auto &p : g) {
    if (p.is_group()) {
      for_all_params(p.as_group(), action);
    } else {
...
template <class P, class... Ps> void set_blocking(bool yes, P &p, Ps &...ps) {
  p.blocking(yes);
  set_blocking(yes, ps...);
}
...
inline group joinable(doc_string docstr, Param param, Params... params) {
  return group{std::move(param), std::move(params)...}.joinable(true).doc(
    std::move(docstr));
}
...
inline group with_prefix(arg_string prefix, Param &&param, Params &&...params) {
  return with_prefix(
    prefix, group{std::forward<Param>(param), std::forward<Params>(params)...});
}
...
inline group &&with_prefixes_short_long(const arg_string &shortFlagPrefix,
                                        ...
                                        group &&params) {
  return std::move(
    with_prefixes_short_long(shortFlagPrefix, longFlagPrefix, params));
}
...
inline group with_suffix(arg_string suffix, Param &&param, Params &&...params) {
  return with_suffix(
    suffix, group{std::forward<Param>(param), std::forward<Params>(params)...});
}

inline parameter &&with_suffixes_short_long(const arg_string &shortsfx,
                                            const arg_string &longsfx,
                                            parameter &&p) {
  return std::move(with_suffixes_short_long(shortsfx, longsfx, p));
}

inline group &with_suffixes_short_long(const arg_string &shortFlagSuffix,
...
inline group &&with_suffixes_short_long(const arg_string &shortFlagSuffix,
                                        ...
                                        group &&params) {
  return std::move(
    with_suffixes_short_long(shortFlagSuffix, longFlagSuffix, params));
}
...
public:
  ...
  explicit scoped_dfs_traverser(const group &g)
    : pos_{g}, lastMatch_{}, posAfterLastMatch_{}, scopes_{},
      ignoreBlocks_{false}, repeatGroupStarted_{false}, repeatGroupContinues_{
                                                          false} {}
  ...
  void next_after_match(scoped_dfs_traverser match) {
    ...
    if (match->blocking()) {
      if (match.pos_.is_alternative()) { match.pos_.skip_alternatives(); }
      ...
      if (is_last_in_current_scope(match.pos_)) {
        ...
        if (!match->repeatable() && !match->is_group()) { ++match.pos_; }
...
match_t full_match(scoped_dfs_traverser pos, const arg_string &arg,
...
template <class ParamSelector>
match_t longest_prefix_match(scoped_dfs_traverser pos, const arg_string &arg,
                             const ParamSelector &select) {
  match_t longest;
...
template <class ParamSelector>
match_t partial_match(scoped_dfs_traverser pos, const arg_string &arg,
                      const ParamSelector &select) {
  while (pos) {
    if (pos->is_param()) {
      const auto &param = pos->as_param();
...
public:
  ...
  public:
    ...
    explicit arg_mapping(arg_index idx, arg_string s,
      ...
      : index_{idx}, arg_{std::move(s)}, match_{match}, repeat_{0},
        startsRepeatGroup_{false}, blocked_{false}, conflict_{false} {}

    explicit arg_mapping(arg_index idx, arg_string s)
      : index_{idx}, arg_{std::move(s)}, match_{}, repeat_{0},
        startsRepeatGroup_{false}, blocked_{false}, conflict_{false} {}
...
public:
  explicit parser(const group &root, arg_index offset = 0)
    : root_{&root}, pos_{root}, index_{offset - 1}, eaten_{0}, args_{},
      missCand_{}, blocked_{false} {
    for_each_potential_miss(
      dfs_traverser{root},
      [this](const dfs_traverser &p) { missCand_.emplace_back(p, index_); });
  }
...
private:
  ...
  void clean_miss_candidates_for(const dfs_traverser &match) {
    auto i = std::find_if(
      missCand_.rbegin(), missCand_.rend(),
      [&](const miss_candidate &m) { return &(*m.pos) == &(*match); });

    if (i != missCand_.rend()) { missCand_.erase(prev(i.base())); }
  }
  ...
  void add_miss_candidates_after(const scoped_dfs_traverser &match) {
    ...
    if (newRepGroup) {
      if (pos_.start_of_repeat_group()) {
        for_each_potential_miss(
          std::move(npos), [&, this](const dfs_traverser &pos) {
...
static parsing_result parse_args(const arg_list &args, const group &cli,
                                 arg_index offset = 0) {
...
static parsing_result parse_and_execute(const arg_list &args, const group &cli,
                                        arg_index offset = 0) {
...
inline parsing_result parse(std::initializer_list<const char *> arglist,
                            const group &cli) {
  arg_list args;
...
inline parsing_result parse(InputIterator first, InputIterator last,
                            const group &cli) {
  return parse(arg_list(first, last), cli);
}

inline parsing_result parse(const int argc, char *argv[], const group &cli,
                            arg_index offset = 1) {
  arg_list args;
...
public:
  ...
  doc_formatting &surround_optional(const string &prefix,
  ...
  doc_formatting &surround_alternatives(const string &prefix,
                                        const string &postfix) {
    alternPre_ = prefix;
    alternPst_ = postfix;
    return *this;
  ...
  doc_formatting &surround_alternative_flags(const string &prefix,
                                             const string &postfix) {
  ...
  doc_formatting &surround_joinable(const string &prefix,
                                    const string &postfix) {
    joinablePre_ = prefix;
...
public:
  ...
  formatting_ostream(OStream &os)
    : os_(os), curCol_{0}, firstCol_{0}, lastCol_{100}, hangingIndent_{0},
      paragraphSpacing_{0}, paragraphSpacingThreshold_{2}, curBlankLines_{0},
      curParagraphLines_{1}, totalNonBlankLines_{0}, ignoreInputNls_{false} {}
  ...
  void wrap_hard(int times = 1) {
    ...
    if (at_begin_of_line()) { ++curBlankLines_; }
    curCol_ = 0;
  ...
  int current_line_begin() const noexcept {
    return in_hanging_part_of_paragraph() ? firstCol_ + hangingIndent_
                                          : firstCol_;
  }
...
private:
  ...
  template <class Iter> bool only_whitespace(Iter first, Iter last) const {
    return last == std::find_if_not(
                     first, last, [](char_type c) { return std::isspace(c); });
  }
  ...
  template <class Iter> void write(Iter first, Iter last) {
  ...
  template <class Iter> void write_line(Iter first, Iter last) {
    if (first == last) return;
    ...
    if (at_begin_of_line()) {
      ...
      first =
        std::find_if(first, last, [](char_type c) { return !std::isspace(c); });
      if (first == last) return;
    }
...
private:
  ...
  template <class OStream> void write(OStream &os) const {
  ...
  void start_group(std::ostringstream &os, const group &group,
                   context &cur) const {
    ...
    if (!lbl.empty()) {
      ...
      if (splitAlternatives) {
        ...
        for (std::size_t i = 0; i < group.size(); ++i) {
          ...
          cur.outermost =
            cur.pos->is_group() ? &(cur.pos->as_group()) : nullptr;
          write(buf, cur, pfx);
          if (buf.tellp() > int(pfx.size())) {
            os << buf.str();
            if (i < group.size() - 1) {
              if (cur.line > 0) { os << string(fmt_.line_spacing(), '\n'); }
              ++cur.line;
              os << '\n';
            }
          }
  ...
  string param_label(const parameter &p, const context &cur) const {
    ...
    if (!flags.empty()) {
      const int n =
        std::min(fmt_.max_flags_per_param_in_usage(), int(flags.size()));
  ...
  string joined_label(const group &g, const context &cur) const {
    ...
    const bool flagsonly =
      std::all_of(g.begin(), g.end(), [](const pattern &p) {
        return p.is_param() && !p.as_param().flags().empty();
      });
  ...
  std::pair<string, string> group_surrounders(const group &group,
                                              const context &cur) const {
    string prefix;
    string postfix;
    ...
    } else if (group.size() > 1 && group.front().blocking() &&
               !group.front().required()) {
      prefix = fmt_.optional_prefix();
      postfix = fmt_.optional_postfix();
  ...
  static string group_separator(const group &group, const doc_formatting &fmt) {
    const bool only1ParamPerMember =
...
public:
  ...
  documentation(const group &cli, const doc_formatting &fmt = doc_formatting{},
...
private:
  ...
  template <class OStream>
  void print_doc(detail::formatting_ostream<OStream> &os, const group &cli,
                 int indentLvl = 0) const {
    if (cli.empty()) return;

    if (cli.doc().empty()) {
      for (const auto &p : cli) { print_doc(os, p, indentLvl); }
    } else {
      bool anyDocInside =
        std::any_of(cli.begin(), cli.end(),
                    [](const pattern &p) { return !p.doc().empty(); });

      if (anyDocInside) {
        handle_spacing(os, paragraph::group, indentLvl);
        os << cli.doc();
        for (const auto &p : cli) { print_doc(os, p, indentLvl + 1); }
      } else {
  ...
  template <class OStream>
  void print_doc(detail::formatting_ostream<OStream> &os, const pattern &ptrn,
                 int indentLvl) const {
    if (ptrn.is_group()) {
  ...
  template <class OStream>
  void handle_spacing(detail::formatting_ostream<OStream> &os, paragraph p,
                      int indentLvl) const {
    ...
    if (p == paragraph::group) {
      if (os.blank_lines_before_paragraph() < fmt_.paragraph_spacing()) {
        os.wrap_hard(fmt_.paragraph_spacing() -
                     os.blank_lines_before_paragraph());
      }
  ...
  template <class OStream>
  void print_entry(detail::formatting_ostream<OStream> &os, const string &label,
                   const string &docstr) const {
  ...
  static doc_string param_label(const parameter &param,
    ...
    if (!flags.empty()) {
      lbl += flags[0];
      const int n =
        std::min(fmt.max_flags_per_param_in_doc(), int(flags.size()));
      for (int i = 1; i < n; ++i) { lbl += fmt.flag_separator() + flags[i]; }
...
inline man_page make_man_page(const group &cli, doc_string progname = "",
                              const doc_formatting &fmt = doc_formatting{}) {
  man_page man;
...
template <class OStream> OStream &operator<<(OStream &os, const man_page &man) {
  bool first = true;
...
template <class OStream> void print(OStream &os, const parsing_result &result) {
...
template <class OStream> void print(OStream &os, const parameter &p) {
  if (p.greedy()) os << '!';
  if (p.blocking()) os << '~';
  if (!p.required()) os << '[';
  os << doc_label(p);
...
template <class OStream> void print(OStream &os, const group &g, int level = 0);
...
template <class OStream> void print(OStream &os, const group &g, int level) {
  auto indent = doc_string(4 * level, ' ');
  os << indent;
  if (g.blocking()) os << '~';
  if (g.joinable()) os << 'J';
  os << (g.exclusive() ? "(|\n" : "(\n");
  for (const auto &p : g) { print(os, p, level + 1); }
  os << '\n' << indent << (g.exclusive() ? "|)" : ")");
  if (g.repeatable()) os << "...";
  os << '\n';
}
